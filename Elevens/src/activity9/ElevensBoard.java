...
public class ElevensBoard extends Board {
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		for(int i = 0; i < selectedCards.size(); i++) {
			for(int j = 0; j < selectedCards.size(); j++) {
				if(cardAt(selectedCards.get(i)).pointValue() + cardAt(selectedCards.get(j)).pointValue() == 11 && i != j)
					return true;
			}
		}
		return false;
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		boolean jack = false; boolean queen = false; boolean king = false;
		for(int card : selectedCards) {
			if(cardAt(card).rank().equals("jack"))
				jack = true;
			else if(cardAt(card).rank().equals("queen"))
				queen = true;
			else if(cardAt(card).rank().equals("king"))
				king = true;
		}
